...
class SyncCommandImpl final : public SyncCommand {
  ...
  FileStreamProvider output_path_file_stream_provider_;
...
void SyncCommandImpl::ChunkReconstructor::EnqueueBlockRetrieval(
  ...
  auto remaining_size =
      Min(parent_impl_.block_, parent_impl_.size_ - begin_offset);
  if (parent_impl_.compression_disabled_) {
...
void SyncCommandImpl::Accept(MetricVisitor &visitor) {
